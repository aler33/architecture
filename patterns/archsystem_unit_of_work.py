...
class UnitOfWork:
    ...
    def insert_new(self):
        ...
        for obj_ in self.new_objects:
            print(f'self.MapperRegistry = {self.MapperRegistry}')
            self.MapperRegistry.get_mapper(obj_).insert(obj_)

    def update_dirty(self):
        for obj_ in self.dirty_objects:
            self.MapperRegitry.get_mapper(obj_).update(obj_)

    def delete_removed(self):
        for obj_ in self.removed_objects:
            self.MapperRegitry.get_mapper(obj_).delete(obj_)
